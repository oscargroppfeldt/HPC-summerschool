...
int main() {
  ...
  HIP_ERRCHK(hipEventRecord(start_a, s_a));
  kernel_a<<<gridsize, blocksize,0,s_a>>>(d_a, N);
  HIP_ERRCHK(hipEventRecord(end_a, s_a));
  HIP_ERRCHK(hipGetLastError());

  HIP_ERRCHK(hipEventRecord(start_b, s_b));
  kernel_b<<<gridsize, blocksize,0,s_b>>>(d_b, N);
  HIP_ERRCHK(hipEventRecord(end_b, s_b));
  HIP_ERRCHK(hipGetLastError());

  HIP_ERRCHK(hipEventRecord(start_c, s_c));
  kernel_c<<<gridsize, blocksize,0,s_c>>>(d_c, N);
  HIP_ERRCHK(hipEventRecord(end_c, s_c));
  HIP_ERRCHK(hipGetLastError());
  ...
  printf("\n");
    ...
    float t_b;
   	float t_c;
	HIP_ERRCHK(hipEventElapsedTime(&t_a, start_a, end_a));
	HIP_ERRCHK(hipEventElapsedTime(&t_b, start_b, end_b));
	HIP_ERRCHK(hipEventElapsedTime(&t_c, start_c, end_c));
  printf("Kernel A elapsed time: %f\n", t_a);
  printf("Kernel B elapsed time: %f\n", t_b);
  printf("Kernel C elapsed time: %f\n", t_c);
